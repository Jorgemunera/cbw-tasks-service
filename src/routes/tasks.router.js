...
// programar tarea para procesar
router.post("/:id/schedule",
    validatorHandler(getTaskByIdSchema, 'params'),
    async (req, res, next) => {
        try {
            const { id } = req.params;
            const result = await service.schedule(id);
...
router.post("/report/completed", async (req, res, next) => {
    try {
        const result = await service.generateCompletedReportJob();
        res.status(202).json(result);
    } catch (error) {
        next(error);
    }
